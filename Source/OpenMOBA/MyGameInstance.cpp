	...
	, bIsLicensed(true) // Default to licensed (should have been checked by OS on boot)
{
	...
	MinimumKillsBeforeResultsSubmit = 1; // When a player has a score of x, submit match results
	RoundWinsNeededToWinMatch = 1; // when a team wins 1 rounds submit match results
		...
		for (int32 b = 0; b < MatchInfo.players.Num(); b++)
		{
			...
			if (MatchInfo.players[b].userKeyId == playerKeyId) {
				...
				else
				{
					...
					// FOr MOBA games, we don't want character storage most likely, but we do want character selection in lobby.
					// So turning this to false here will force a selection from all players in the lobby.
					MatchInfo.players[b].characterCustomized = false;
				}
		...
		if (FJsonSerializer::Deserialize(JsonReader, JsonParsed))
		{
			...
			if (Authorization)
			{
				...
				if (PlayerAuthorized) {
						...
						if (MatchInfo.players[b].userKeyId == JsonParsed->GetStringField("player_userid")) {
							...
							for (FConstPlayerControllerIterator Iterator = GetWorld()->GetPlayerControllerIterator(); Iterator; ++Iterator)
							{
								...
								if (playerS->playerKeyId == JsonParsed->GetStringField("user_key_id"))
								{
									...
									// Normally we would Also set the character customization bool we got from match info
									// But in the case of a MOBA, we want the player to always pick the hero/character
									// We're not actually checking this value in the UI, but if we were for whatever reason, we can force this to false here.
					...
					// We're using getGamePlayer, so we don't want to start the match until that process is complete
	...
	else
	{
		...
		if (FJsonSerializer::Deserialize(JsonReader, JsonParsed))
		{
			...
			if (Authorization)
			{
				...
				//  MOBA don't really need characters.  
				// But we are going to use this character customized and all players char cust complete in the lobby.

				// And this entire check is not needed because we are forcing hero selection in the lobby.
				// So we never need to auto-start the game.
...
void UMyGameInstance::AttemptStartMatch()
{
	...
	if (IsRunningDedicatedServer())
	{
		...
		if (allPlayersJoined && allPlayersCharCustComplete)
		{
			// Calculate the average team ranks
			...
			FString UrlString = TEXT("/Game/Maps/Game?listen");
			if (MatchInfo.gameModeTitle == "1v1")
			{
				...
				UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [UMyGameInstance] [GetMatchInfo] gameModeTitle != 1v1"));
				// For everything else, travel to the other map - just an example.
				UrlString = TEXT("/Game/Maps/Game?listen");
			}
