...
void AMyPlayerController::SaveCharacterCustomization(FString meshSelection, FString textureSelection)
{
	...
	// In OpenMoba "Champions" are stored as an enum read from a datafile.
	// But this datafile was defined in bp, so we don't actually have access to it here.
	// If you are planning on actually using a datafile for this purpose, I recommend 
	// Declaring the enum in c++ and referencing it here.
	// Otherwise, do something else here.
	// For now, just hardcoding it.
	if (meshSelection == "Champion")
	{
		MeshIndex = 0;
	}
	else
...
void AMyPlayerController::SaveCharacterCustomizationServer_Implementation(int32 meshSelection, int32 textureSelection)
{
	...
	// Tell the openMOBA BP to load the latest data
	ReceiveRecievePlayerState();
