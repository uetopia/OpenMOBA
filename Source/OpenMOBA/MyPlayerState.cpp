...
{
	UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [AMyPlayerState] OnRep_OnCustomTextureChange_Implementation."));
	...
	{
		UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [AMyPlayerState] OnRep_OnCustomTextureChange_Implementation.  customTextures.Num != LoadedTextures.Num"));
		...
		for (int32 b = 0; b < customTextures.Num(); b++) {
			...
			{
				UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [AMyPlayerState] OnRep_OnCustomTextureChange_Implementation.  INVALID FILENAME"));
...
{
	UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [AMyPlayerState] ReadCustomTexture_HttpRequestComplete."));
	if (!HttpRequest.IsValid())
	{
		UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [AMyPlayerState] ReadCustomTexture_HttpRequestComplete REQUEST INVALID."));
		return;
	}
	...
	*/
	...
	{
		UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [AMyPlayerState] ReadCustomTexture_HttpRequestComplete. bSucceeded and IsValid "));
		if (EHttpResponseCodes::IsOk(HttpResponse->GetResponseCode()))
		{
			...
			UE_LOG(LogTemp, Warning, TEXT("[UETOPIA] [AMyPlayerState] ReadCustomTexture_HttpRequestComplete. TexturePath: %s"), *TexturePath);
			TArray<uint8> FileData;

			if (FFileHelper::LoadFileToArray(FileData, *TexturePath))
			{
				UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [AMyPlayerState] ReadCustomTexture_HttpRequestComplete. LoadFileToArray "));
				if (ImageWrapper.IsValid() && ImageWrapper->SetCompressed(FileData.GetData(), FileData.Num()))
				{
					UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [AMyPlayerState] ReadCustomTexture_HttpRequestComplete. ImageWrapper->SetCompressed "));
					const TArray<uint8>* UncompressedBGRA = NULL;
					if (ImageWrapper->GetRaw(ERGBFormat::BGRA, 8, UncompressedBGRA))
					{
						UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [AMyPlayerState] ReadCustomTexture_HttpRequestComplete. ImageWrapper->GetRaw "));
...
{
	UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [AMyPlayerState] LoadTexturesOntoActors_Implementation."));
	...
	while (AllActorsItr)
	{
		UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [AMyPlayerState] LoadTexturesOntoActors_Implementation. Found ADynamicTextureStaticMeshActor "));

		if (AllActorsItr->MaterialInstance->IsValidLowLevel())
		{
			UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [AMyPlayerState] LoadTexturesOntoActors_Implementation. Material Instance IsValid "));
...
void AMyPlayerState::CopyProperties(class APlayerState* PlayerState)
{
	...
	if (MyPlayerState)
	{
		...
		MyPlayerState->LoadedTextures = LoadedTextures;
