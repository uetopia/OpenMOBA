...
UCLASS()
class OPENMOBA_API AMyPlayerState : public APlayerState
{
	GENERATED_BODY()
...
public:
	...
	*/
	...
	// For some reason in OpenMOBA our Repnotify is never called 
	// Setting this up so we can call it manually to force a reload
	// This still does not work correctly.
	UFUNCTION(Client, BlueprintCallable, Category = "UETOPIA", Reliable)
		virtual void OnRep_OnCustomTextureChange();

	UFUNCTION(BlueprintCallable, Category = "UETOPIA", Client, Reliable)
		virtual void LoadTexturesOntoActors();

	// OpenMOBA does not handle this client side texturing correctly for some reason.
	UPROPERTY()
